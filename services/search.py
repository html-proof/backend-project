...
class SearchService:
    ...
    def get_personal_context(self, user_id: str) -> Dict[str, Any]:
        ...
        history = firebase_db.get_play_history(user_id, limit=30)
        
        liked_artists = {self.normalize(s.get('artist', '')) for s in liked if s.get('artist')}
        # Skips are also relevant
        # Interactions and skips not yet implemented
        skipped_artists = set()
    ...
    async def search_songs(self, query: str, limit: int = 10, user_id: str = None) -> List[Dict[str, Any]]:
        ...
        # 2. Get User Context
        try:
            # firebase calls are sync (firebase-admin), but fast enough. 
            # ideally should be in executor too if slow.
            # For now, let's assume they are fast or we can wrap them later.
        ...
        try:
            ...
            for entry in entries:
                ...
                # Popularity boost
                if view_count > 10000000: score += 20
                elif view_count > 1000000: score += 10
